...
def distGlobal(latA, lonA, latB, lonB):
    ...
    # distance
    dist = math.acos(math.sin(latA) * math.sin(latB) + math.cos(latA) * math.cos(latB) * math.cos((lonB - lonA)))
...
class EvaluateEvent(object):
    ...
    SLAB_DATA_PATH = "assets/data/slab/models"
    ...
    def readSlabPerimeters(self):
        ...
        for slab in EvaluateEvent.SLAB_LIST:
            filename = os.path.join(EvaluateEvent.SLAB_DATA_PATH, slab[1])
            polygon = []
    ...
    def isInsideSlabPerimeter(self, lat, lon):
        ...
        if len(self.perim_polygons) < len(EvaluateEvent.SLAB_LIST):
    ...
    def isInsideSlab(self, lat, lon):
        ...
        if is_inside_slab:
            slab_depth = -9.9
            slab_name = EvaluateEvent.SLAB_LIST[n_slab][0]
            try: 
                fname_in_temp = "temp_grdtrack_in.txt"
                f_in = open(fname_in_temp, "w")
                f_in.write(str(lon) + " " + str(lat) + " \n")
                f_in.close()
                filename = os.path.join(EvaluateEvent.SLAB_DATA_PATH, EvaluateEvent.SLAB_LIST[n_slab][2])
    ...
    def percentProbIsUnderwater(self, lat, lon, minHorUnc, maxHorUnc, azMaxHorUnc):
        ...
        filename = os.path.join(EvaluateEvent.UNDERWATER_DATA_PATH, "percent_prob_underwater.hdr")
        ...
        filename = os.path.join(EvaluateEvent.UNDERWATER_DATA_PATH, "percent_prob_underwater.table")
        ...
        if maxHorUnc > 0.0:
            nshell = 1
            scale2km = degree_step * EvaluateEvent.DEG2KM
    ...
    def findClosestPlateBoundary(self, lat, lon, exclude_boundary_codes):
        ...
        for plate_boundary in EvaluateEvent.PLATE_BOUNDARY_LIST:
            filename = os.path.join(EvaluateEvent.PLATE_BOUNDARY_DATA_PATH, plate_boundary[0])
            #print "DEBUG: read:", filename
            f_perim = open(filename, "r")
            flines = f_perim.readlines()
            for line in flines:
        ...
        return distance_min * EvaluateEvent.DEG2KM, closest_boundary_code, closest_boundary_class, closest_boundary_class_id
...
#         logger.info("Probability underwater:", int(0.5 + percent_mean), "+/-", int(0.5 + percent_std_dev), "%")
#
#     # plate boundary
#     if True:
#
...
#             logger.info("In a slab:", slab_name + ": slab depth:", slab_depth_str, "km")
#         else:
#             logger.info("Not in a slab.")
